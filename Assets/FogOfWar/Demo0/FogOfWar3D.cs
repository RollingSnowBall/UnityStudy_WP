...
public class FogOfWar3D : MonoBehaviour
{
	...
	public List<Transform> m_listClearZone;
	...
	public void MakeClear(){
		Vector3 rangePoint = new Vector3(Random.Range(m_topLeft.position.x, m_topRight.position.x), 
			0,
			Random.Range(m_bottomLeft.position.z, m_topLeft.position.z));

		if(null != m_fogOfWarMaterial){
			Vector4 viewCenter0 = TransformWorldCoordToUVCoord(rangePoint);
			viewCenter0.z = Random.Range(0, 1.0F);
			viewCenter0.w = Random.Range(0, 1.0F);
			m_fogOfWarMaterial.SetVector("_ViewCenter", viewCenter0);
